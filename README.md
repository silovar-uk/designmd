...
HTML版は、designmd自身の原則を使って再設計している。

1. **問い**：AIと人間でブリーフを監査する
2. **工程**：定める・集める・組み立てる・つくる・確かめる
3. **媒体**：文章・スライド・画像で形式を変える
4. **レビュー**：問い・材料・構造・表現・責任を確認する
5. **使う**：必要なプロンプトと正典だけを選ぶ

旧サイトの8節とv2〜v9の増築レイヤーは、公開ページの読み込み対象から外した。現在の公開面は`site/index.html`、`site/styles.css`、`site/app.js`の三つを正とする。旧ファイルは履歴参照のため残している。
