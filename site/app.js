(() => {
  ...
  if (checklist && score && message) {
    ...
    const updateScore = () => {
      ...
      if (checked === 0) {
        message.textContent = 'まずは該当箇所を選んでください。';
      } else if (checked <= 5) {
        message.textContent = '局所的な修正で改善できそうです。根拠と具体性を確認してください。';
      } else if (checked <= 10) {
        message.textContent = '構造の反復が出ています。ページや段落の役割から見直してください。';
      } else if (checked <= 15) {
        message.textContent = '見た目より前の工程へ戻る段階です。原材料と判断主体を確認してください。';
      } else {
        message.textContent = 'テイスト変更では解決しません。目的・原材料・構成から再設計してください。';
      }
    };
